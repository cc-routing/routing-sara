...
public class UndirectedGraphTest {
    ...
    private UndirectedGraph createGraph() {
        Map<Node, ArrayList<Edge>> map = new HashMap<>();
        List<Node> nodes = new ArrayList<>();
        Node a = createNode( nodes, map, 0 );
        Node b = createNode( nodes, map, 1 );
        Node c = createNode( nodes, map, 2 );
        Node d = createNode( nodes, map, 3 );
        Node e = createNode( nodes, map, 4 );
        Node f = createNode( nodes, map, 5 );
        List<Edge> edges = new ArrayList<>();
        Edge ab = createEdge( edges, map, 0, false, a, b, 120 );
        Edge ac = createEdge( edges, map, 1, false, a, c, 184 );
        Edge cd = createEdge( edges, map, 2, false, c, d, 94 );
        Edge db = createEdge( edges, map, 3, true, d, b, 159 );
        Edge be = createEdge( edges, map, 4, false, b, e, 36 );
        Edge df = createEdge( edges, map, 5, false, d, f, 152 );
        Edge ef = createEdge( edges, map, 6, true, e, f, 38 );
        UndirectedGraph g = new UndirectedGraph( nodes, edges );
        return g;
    }

    private Node createNode( List<Node> nodes, Map<Node, ArrayList<Edge>> map, long id ) {
    ...
    private Edge createEdge( List<Edge> edges, Map<Node, ArrayList<Edge>> map, long id, boolean oneway, Node source, Node target, double distance ) {
        Edge edge = new Edge( id, oneway, source, target, Distance.newInstance( distance ) );
        edges.add( edge );
        map.get( source ).add( edge );
        map.get( target ).add( edge );
        edgeMap.put( id, edge );
        return edge;
    }
