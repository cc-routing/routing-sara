...
public class MLDRecursiveRouteUnpacker<N extends Node<N, E>, E extends Edge<N, E>> implements RouteUnpacker<N, E> {
        ...
        if ( endPoint != null ) {
            ...
            while ( currentState != null && !currentState.isFirst() ) {
                if ( !( currentState.getNode() instanceof OverlayNode ) ) {
                    ...
                    unpackHighLevels( builder, saraDijkstra, overlayDijkstra, oTo.getLevel(), overlayGraph, metric, (E) oFrom.getIncomingEdges().next(), (E) oTo.getOutgoingEdges().next() );
                }
                currentState = predecessors.get( currentState );
            }
    ...
    private void unpackHighLevels( Route.RouteBuilder<N, E> builder, RoutingAlgorithm saraDijkstra, RoutingAlgorithm overlayDijkstra, int level, OverlayBuilder overlayGraph, Metric metric, E fromEdge, E toEdge ) {
        if ( level == 1 ) {
            ...
            for ( int i = subRoute.getEdgeList().size() - 1; i >= 0; i -= 2 ) {
                ...
                unpackHighLevels( builder, saraDijkstra, overlayDijkstra, level - 1, overlayGraph, metric, subRoute.getEdgeList().get( i - 2 ), subRoute.getEdgeList().get( i ) );
            }
        }
    }
