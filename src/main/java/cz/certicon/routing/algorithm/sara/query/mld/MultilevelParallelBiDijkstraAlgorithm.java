...
public class MultilevelParallelBiDijkstraAlgorithm implements RoutingAlgorithm<SaraNode, SaraEdge> {
     ...
     */
    public MultilevelParallelBiDijkstraAlgorithm( OverlayBuilder overlayBuilder, RouteUnpacker unpacker ) {
...
//                    System.out.println( "MLD - final state >>> " + nodeDistanceMap.get( state ) );
    ...
    private <T extends Node> boolean updateQueue(
        ...
        if ( !closedStates.contains( transferState ) ) {
            ...
            Distance alternativeDistance = currentDistance.add( edge.getLength( metric ) ).add( currentState.isFirst() ? Distance.newZeroDistance() : currentState.getNode().getTurnDistance( currentState.getEdge(), edge ) );
