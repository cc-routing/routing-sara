...
public class DisplayUtils {
    ...
    public static <N extends Node, E extends Edge> void display( Graph<N, E> graph, Collection<ElementContainer<N>> nodeGroups ) {
        ...
        for ( ElementContainer<N> nodeGroup : nodeGroups ) {
            for ( N n : nodeGroup ) {
                SimpleNode node = new SimpleNode( n.getId() );
        ...
        TLongObjectMap<SimpleEdge> edgeMap = new TLongObjectHashMap<>();
        for ( E e : graph.getEdges() ) {
            Node s = e.getSource( graph );
            Node t = e.getTarget( graph );
            if ( !edgeMap.containsKey( e.getId() ) && nodeMap.containsKey( s.getId() ) && nodeMap.containsKey( t.getId() ) ) {
                SimpleNode source = nodeMap.get( s.getId() );
                SimpleNode target = nodeMap.get( t.getId() );
                SimpleEdge edge = new SimpleEdge( e.getId(), false, source, target, 0, 0 );
                source.addEdge( edge );
                target.addEdge( edge );
                edgeMap.put( edge.getId(), edge );
            }
        }
        Graph<SimpleNode, SimpleEdge> g = new UndirectedGraph<>( nodeMap.valueCollection(), edgeMap.valueCollection(), metricMap );
        for ( E e : graph.getEdges() ) {
            if ( edgeMap.containsKey( e.getId() ) ) {
                SimpleEdge edge = edgeMap.get( e.getId() );
                g.setLength( Metric.SIZE, edge, graph.getLength( Metric.SIZE, e ) );
            }
        }
